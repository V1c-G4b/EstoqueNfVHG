...
interface TableToolbarProps {
  ...
  onStatusFilter: (
    status: "all" | "autorizada" | "cancelada" | "rejeitada"
  ) => void;
  searchValue: string;
  statusFilter: "all" | "autorizada" | "cancelada" | "rejeitada";
  resultsCount: number;
  totalCount: number;
}
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              <DropdownMenuCheckboxItem
                checked={statusFilter === "autorizada"}
                onCheckedChange={() => onStatusFilter("autorizada")}
              >
                Autorizadas
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem
                checked={statusFilter === "cancelada"}
                onCheckedChange={() => onStatusFilter("cancelada")}
              >
                Canceladas
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem
                checked={statusFilter === "rejeitada"}
                onCheckedChange={() => onStatusFilter("rejeitada")}
              >
                Rejeitadas
              </DropdownMenuCheckboxItem>
            </DropdownMenuContent>
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <div>
          Mostrando {resultsCount} de {totalCount} nota
          {totalCount !== 1 ? "s" : ""}
          {hasActiveFilters && (
            <span className="ml-1">
              (filtrada{resultsCount !== 1 ? "s" : ""})
            </span>
          )}
        </div>
